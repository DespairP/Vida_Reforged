...
public class SelectButton<T> extends VidaWidget {
    /**按钮大小*/
    ...
    public SelectButton<T> setIconFromColor(ARGBColor iconFromColor){
        this.iconFromColor = iconFromColor;
        return this;
    }

    public SelectButton<T> setIconToColor(ARGBColor iconToColor) {
        this.iconToColor = iconToColor;
        return this;
    }

    public SelectButton(int x, int y, int width, int height, T id) {
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        consumer.vertex(matrix4f, 0, 0, 12).color(fromR, fromG, fromB, alpha).endVertex();
        consumer.vertex(matrix4f, 0, getHeight(), 12).color(fromR, fromG, fromB, alpha).endVertex();
        consumer.vertex(matrix4f, getWidth(), getHeight(), 12).color(toR, toG, toB, alpha).endVertex();
        consumer.vertex(matrix4f, getWidth(), 0, 12).color(toR, toG, toB, alpha).endVertex();
        poseStack.popPose();
    ...
    public void renderFocused(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks){
        ...
        consumer.vertex(matrix4f, 0, 0, 13).color(toR, toG, toB, 1).endVertex();
        consumer.vertex(matrix4f, 0, getHeight(), 13).color(fromR, fromG, fromB, 1).endVertex();
        consumer.vertex(matrix4f, 2, getHeight(), 13).color(fromR, fromG, fromB, 1).endVertex();
        consumer.vertex(matrix4f, 2, 0, 13).color(toR, toG, toB, 1).endVertex();
        poseStack.popPose();
    }
