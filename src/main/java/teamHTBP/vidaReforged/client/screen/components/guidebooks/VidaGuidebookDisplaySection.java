...
public class VidaGuidebookDisplaySection extends VidaWidget implements IVidaGuidebookComponent  {
    ...
    public VidaGuidebookDisplaySection(int x, int y, int width, int height, ItemStack renderItem) {
        super(x, y, width, height, Component.literal("model"));
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        renderBlockText(graphics, itemStack.getItem().getName(itemStack), textX, textY, partialTicks);
    }
    ...
    protected void renderBlockText(GuiGraphics graphics,Component component, int x, int y, float partialTicks) {
        ...
        poseStack.pushPose();
        graphics.drawCenteredString(font, component, x + 40, y + 30, ARGBColor.of(hoverRange.get() + 0.4f,1,1, 1).argb());
        poseStack.popPose();
    }
    ...
    protected void renderBg(GuiGraphics graphics, int x, int y, int width, int height, float partialTicks){
        ...
        graphics.blit(
                GUIDEBOOK_RESOURCE,
                x, y + 3, 0,
                BACKGROUND_SEC.minU(), BACKGROUND_SEC.minV(),
                BACKGROUND_SEC.w(), BACKGROUND_SEC.h(),
                512, 512
        );
        poseStack.popPose();
    }
    ...
    public void renderWithState(GuiGraphics graphics, ItemStack renderStack, int startX, int startY, float partialTicks){
        ...
        pPoseStack.pushPose();
        pPoseStack.translate(model.isGui3d()? 16.0F + 8.0F + startX : startX, model.isGui3d() ? 16.0F + 8.0F + startY : startY + 32.0F, 16.0F);
